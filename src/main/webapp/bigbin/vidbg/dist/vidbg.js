...
!(function(root, factory) {
  ...
  Vidbg.prototype.init = function() {
    ...
    try {
      $video.prop({ // Set video properties
    ...
    $video.css({
      ...
      visibility: 'hidden',
      opacity: 0
    }).one('canplaythrough.' + PLUGIN_NAME, function() {// Resize a video, when it's loaded
      vidbg.resize();
    }).one('playing.' + PLUGIN_NAME, function() {// Make it visible, when it's already playing
      $video.css({
        visibility: 'visible',
        opacity: 1
      });
